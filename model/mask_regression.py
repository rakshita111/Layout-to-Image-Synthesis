...
class MaskRegressNetv2(nn.Module):
    def __init__(self, num_parts, obj_feat=128, mask_size=16, map_size=64):
        ...
        conv3.append(nn.ReLU())
        conv3.append(nn.utils.spectral_norm(nn.Conv2d(256, 4, 3, 1, 1)))
        conv3.append(nn.Sigmoid())
        self.conv3 = nn.Sequential(*conv3)
    ...
    def forward(self, obj_feat, bbox):
        ...
        :param obj_feat: (b*num_o, feat_dim)
        :param bbox: (b, num_o, 4)
        :return: bbmap: (b, num_o, num_parts, map_size, map_size)
        """
        try:
            b, num_o, p, _ = bbox.size()
        except:
            b, num_o, _ = bbox.size()
            p = 1

        obj_feat = obj_feat.view(b * num_o * p, -1)
        x = self.fc(obj_feat)
        x = self.conv1(x.view(b * num_o * p, 256, 4, 4))
        x = F.interpolate(x, size=8, mode='bilinear')
        x = self.conv2(x)
        x = F.interpolate(x, size=16, mode='bilinear')
        x = self.conv3(x)
        x = x.view(b, num_o, self.np, 16, 16)

        bbmap = masks_to_layout(bbox, x, self.map_size).view(b, num_o, self.np, self.map_size, self.map_size)

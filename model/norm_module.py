...
class SpatialAdaptiveBatchNorm2d(nn.Module):
    def __init__(self, num_features, num_w=512, batchnorm_func=BatchNorm2d, eps=1e-5, momentum=0.1, affine=False,
...
class SpatialAdaptiveSynBatchNorm2d(nn.Module):
    ...
    def forward(self, x, vector, bbox):
        ...
        :param x: input feature map (b, c, h, w)
        :param vector: latent vector (b*o, dim_w)
        :param bbox: bbox map (b, o, p, h, w)
        :return:
        """
        # self._check_input_dim(x)
        output = self.batch_norm2d(x)

        b, o, bp, bh, bw = bbox.size()
        vector = vector.unsqueeze(1).repeat(1, bp, 1).view(b * o * bp, -1)
        _, _, h, w = output.size()
        if bh != h or bw != w:
            bbox = F.interpolate(bbox, size=(bp, h, w), mode='trilinear')
        bbox = bbox.view(b, -1, h, w)

        # calculate weight and bias
        weight, bias = self.weight_proj(vector), self.bias_proj(vector)

        weight, bias = weight.view(b, o*bp, -1), bias.view(b, o*bp, -1)

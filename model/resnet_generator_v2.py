...
class ResnetGenerator128(nn.Module):
    def __init__(self, p, ch=64, z_dim=128, num_classes=10, output_dim=3):
        ...
        self.res1 = ResBlock(ch*16, ch*16*p, num_parts=p, upsample=True, num_w=num_w)
        self.res2 = ResBlock(ch*16*p, ch*8*p, num_parts=p, upsample=True, num_w=num_w)
        self.res3 = ResBlock(ch*8*p, ch*4*p, num_parts=p, upsample=True, num_w=num_w)
        self.res4 = ResBlock(ch*4*p, ch*2*p, num_parts=p, upsample=True, num_w=num_w, psp_module=True)
        self.res5 = ResBlock(ch*2*p, ch*p, num_parts=p, upsample=True, num_w=num_w, predict_mask=False)
        self.final = nn.Sequential(BatchNorm(ch*p),
                                   nn.ReLU(),
                                   conv2d(ch*p, output_dim, 3, 1, 1),
                                   nn.Tanh())
        ...
        self.alpha1 = nn.Parameter(torch.zeros(1, 184, 1, 1))
        self.alpha2 = nn.Parameter(torch.zeros(1, 184, 1, 1))
        self.alpha3 = nn.Parameter(torch.zeros(1, 184, 1, 1))
        self.alpha4 = nn.Parameter(torch.zeros(1, 184, 1, 1))

        self.sigmoid = nn.Sigmoid()

        self.attn = AttnBlock(ch*8*p)
        self.mask_regress = MaskRegressNetv2(p, num_w)
        self.init_parameter()
    ...
    def forward(self, z, bbox, z_im=None, y=None):
        ...
        bbox_mask_ = bbox_mask(z, bbox, 64, 64).unsqueeze(2).repeat(1, 1, p, 1, 1)
        ...
        stage_mask = stage_mask.view(b, -1, p, hh, ww)

        # 16x16

        seman_bbox = batched_index_select(stage_mask, dim=1, index=y.view(b, o, p, 1, 1)) # size (b, num_o, h, w)
        seman_bbox = torch.sigmoid(seman_bbox) * F.interpolate(bbox_mask_, size=(p, hh, ww), mode='nearest')
        alpha1 = torch.gather(self.sigmoid(self.alpha1).expand(b, -1, p, -1), dim=1, index=y.view(b, o, p, 1)).\
            unsqueeze(-1)
        stage_bbox = F.interpolate(bmask, size=(p, hh, ww), mode='trilinear') * (1 - alpha1) + seman_bbox * alpha1
        ...
        seman_bbox = batched_index_select(stage_mask, dim=1, index=y.view(b, o, p, 1, 1)) # size (b, num_o, h, w)
        seman_bbox = torch.sigmoid(seman_bbox) * F.interpolate(bbox_mask_, size=(p, hh, ww), mode='nearest')
        alpha2 = torch.gather(self.sigmoid(self.alpha2).expand(b, -1, p, -1), dim=1, index=y.view(b, o, p, 1)).\
            unsqueeze(-1)
        stage_bbox = F.interpolate(bmask, size=(p, hh, ww), mode='trilinear') * (1 - alpha2) + seman_bbox * alpha2
        ...
        seman_bbox = batched_index_select(stage_mask, dim=1, index=y.view(b, o, p, 1, 1)) # size (b, num_o, h, w)
        seman_bbox = torch.sigmoid(seman_bbox) * F.interpolate(bbox_mask_, size=(p, hh, ww), mode='nearest')
        alpha3 = torch.gather(self.sigmoid(self.alpha3).expand(b, -1, p, -1), dim=1, index=y.view(b, o, p, 1)).\
            unsqueeze(-1)
        stage_bbox = F.interpolate(bmask, size=(p, hh, ww), mode='trilinear') * (1 - alpha3) + seman_bbox * alpha3
        ...
        seman_bbox = batched_index_select(stage_mask, dim=1, index=y.view(b, o, p, 1, 1)) # size (b, num_o, h, w)
        seman_bbox = torch.sigmoid(seman_bbox) * F.interpolate(bbox_mask_, size=(p, hh, ww), mode='nearest')
        alpha4 = torch.gather(self.sigmoid(self.alpha4).expand(b, -1, p, -1), dim=1, index=y.view(b, o, p, 1)).\
            unsqueeze(-1)
        stage_bbox = F.interpolate(bmask, size=(p, hh, ww), mode='trilinear') * (1 - alpha4) + seman_bbox * alpha4
...
class ResBlock(nn.Module):
    def __init__(self, in_ch, out_ch, num_parts, h_ch=None, ksize=3, pad=1, upsample=False, num_w=128, predict_mask=True, psp_module=False):
        ...
        if self.predict_mask:
            if psp_module:
                self.conv_mask = nn.Sequential(PSPModule(out_ch, 100),
                                               nn.Conv2d(100, 184*num_parts, kernel_size=1))
            else:
                self.conv_mask = nn.Sequential(nn.Conv2d(out_ch, 100, 3, 1, 1),
                                               BatchNorm(100),
                                               nn.ReLU(),
                                               nn.Conv2d(100, 184*num_parts, 1, 1, 0, bias=True))
